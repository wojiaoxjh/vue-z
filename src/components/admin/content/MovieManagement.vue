...
export default {
  ...
  methods: {
    loadMovies () {
      const _this = this;
      this.$axios.get('http://localhost:9090/movies/page/0').then(resp => {
    ...
    handleCurrentChange: function (currentPage) {
      const url = 'http://localhost:9090/movies/page/' + (currentPage*this.pageSize);
    ...
    deleteMovie (id) {
      this.$confirm('此操作将永久删除该电影信息, 是否继续?', '提示', {
        ...
        type: 'warning'
            ...
            this.$axios
                .post('http://localhost:9090/admin/content/movie/delete', {id: id}).then(resp => {
              if (resp && resp.status === 200) {
                this.loadMovies()
              }
            })
